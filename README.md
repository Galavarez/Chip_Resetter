...
4. В базе данных (Struct_DB datebase) можно закомментировать двойным слешем // строки с чипами которые вы не используете и обнулятор их не покажет.
5. По копке SELECT запускаются подпрограммы:
* Sizing button (калибровка кнопок) -- Нажимаем кнопки по очереди и записываем эти значения в скетч строки 875-879, теперь не будет "случайных" нажатий. Выход из подпрограммы кнопкой RESET.
