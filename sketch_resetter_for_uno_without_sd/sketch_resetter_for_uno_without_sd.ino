...
* Версия 15.08.2025
- Переписал функцию извлечения дампа (extract_dump), теперь она показывает красивый дамп. 
	Так же можно копировать дамп в блокнот и сохранить его с расширением hex.
	Получим дамп с чипа	а потом загрузить его в EasyAddDump или любой другой HEX редактор.
	Дамп чипа находится между словами START END.
...
{   
  ...
  lcd.clear();
  lcd.print(F("CHIP RESETTER"));
  lcd.setCursor(0,1);
	lcd.print(F("VER 15/08/2025"));
  delay(1500);
...
{  
  ...
  switch ( button(BUTTON_SELECT) )
  {
    ...
    case 1: 
      ...
      if ( global_button_select > 8 ) // 8 это кол-во подпрограмм
      {
    ...
    case 2:
     ...
     switch(global_button_select)
     {
      ...
      case 4:
        ...
        break;				
      case 6:
        extract_dump(1024);   // Показываем в мониторе порта дамп с чипа первые 1024 байт         
        break;
      case 7:
...
/************************************* ПОКАЗ ДАМПА В МОНИТОРЕ ПОРТА МОДИФИКАЦИЯ 2025-08-15 */
// Калькулятор https://fischl.de/hex_checksum_calculator/
void extract_dump(word SizeDump)
{ 
  lcd.clear();
  lcd.print(F("READ DUMP ON PC"));
  ...
  if (search_address_chip())
  {
      ...
      for (int Cycle = 0; Cycle < CalculateCycles;  Cycle++)
      {
        // Показываем HEX адреса колонок
				...
				if (HigherByte < 16) { Serial.print("0"); } Serial.print(HigherByte, HEX);				
				...
				if (LowerByte < 16) { Serial.print("0"); } Serial.print(LowerByte, HEX);        
        ...
        {  
          ...
          // Если это число меньше 16 то добавляем 0 спереди и показываем в HEX
					if (GetByte < 16) { Serial.print(F("0")); } Serial.print(GetByte, HEX);	
					// Отступ
          Serial.print(F(" "));
        }
			...
			for (int Cycle = 0; Cycle < CalculateCycles;  Cycle++)
      {
        ...
        {  
          ...
          // Если это число меньше 16 то добавляем 0 спереди и показываем в HEX
					if (GetByte < 16) { Serial.print("0"); } Serial.print(GetByte, HEX); 				
        }
				// 1 байт CheckSum 
				...
				// Если это число меньше 16 то добавляем 0 спереди и показываем в HEX
				if (FinalCheckSum < 16) { Serial.print("0"); } Serial.print(FinalCheckSum, HEX); 
				// Переход на новую строку
        Serial.println(F("")); 
      }
			// Конец HEX файла
...
void WriteBytesPages(byte AddressDevice, word SizeDump, byte *PointerToDump) 
{	
...
// AddressDevice -- Адрес чипа
// NumberCell -- Номер ячейки
byte ReadByte(byte AddressDevice, word NumberCell) 
{	
